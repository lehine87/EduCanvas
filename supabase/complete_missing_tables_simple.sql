-- ================================================================
-- EduCanvas T-003 완료를 위한 누락 테이블 추가 스크립트 (단순화)
-- 작성일: 2025-08-10
-- ================================================================

-- ENUM 타입들 추가
CREATE TYPE IF NOT EXISTS attendance_status AS ENUM ('present', 'absent', 'late', 'excused');
CREATE TYPE IF NOT EXISTS payment_status AS ENUM ('pending', 'completed', 'overdue', 'cancelled', 'refunded');
CREATE TYPE IF NOT EXISTS billing_type AS ENUM ('monthly', 'sessions', 'hours', 'package', 'drop_in');
CREATE TYPE IF NOT EXISTS salary_policy_type AS ENUM ('fixed_monthly', 'fixed_hourly', 'commission', 'tiered_commission', 'student_based', 'hybrid', 'guaranteed_minimum');
CREATE TYPE IF NOT EXISTS video_status AS ENUM ('draft', 'published', 'private', 'archived', 'deleted');
CREATE TYPE IF NOT EXISTS video_type AS ENUM ('lecture', 'supplement', 'homework_review', 'exam_review', 'announcement');
CREATE TYPE IF NOT EXISTS watch_status AS ENUM ('not_started', 'in_progress', 'completed', 'skipped');
CREATE TYPE IF NOT EXISTS video_quality AS ENUM ('240p', '360p', '480p', '720p', '1080p', '1440p', '2160p');
CREATE TYPE IF NOT EXISTS consultation_status AS ENUM ('scheduled', 'completed', 'cancelled', 'no_show');
CREATE TYPE IF NOT EXISTS consultation_type AS ENUM ('enrollment', 'academic', 'behavioral', 'career', 'parent_meeting', 'follow_up');
CREATE TYPE IF NOT EXISTS history_action AS ENUM ('create', 'update', 'delete', 'move', 'enroll', 'withdraw', 'payment', 'exam', 'consultation');
