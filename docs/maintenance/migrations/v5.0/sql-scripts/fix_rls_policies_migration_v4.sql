-- =====================================================
-- Fix RLS Policies Migration v4
-- Corrected RLS policies with proper students-classes relationship
-- =====================================================

-- Step 1: Try to drop constraint safely (ignore if doesn't exist)
DO $$ 
BEGIN
    BEGIN
...
-- Step 2: Drop the old instructor_id column with CASCADE
-- This will automatically drop all dependent RLS policies
ALTER TABLE classes DROP COLUMN instructor_id CASCADE;
...
-- Step 5: Recreate RLS policies with updated logic for the new architecture
...
-- Student video access policy (instructors can access their class students' videos)
-- Corrected: students and classes are linked through student_enrollments
...
-- Step 6: Show what policies were recreated
SELECT 
    schemaname,
    tablename,
    policyname,
    'Recreated with corrected architecture' as status
FROM pg_policies 
WHERE policyname LIKE '%instructor%'
AND tablename IN ('attendances', 'student_video_access');
