...
const searchStudents = (students: Student[], query: string): SearchResult[] => {
  ...
  students.forEach(student => {
    const searchResult: Partial<SearchResult> = {
      ...
      grade_level: (student as any).grade_level || (student as any).grade,
      status: student.status as string,
      avatar_url: (student as any).avatar_url || (student as any).photo_url
    }
    ...
    if (((student as any).grade_level || (student as any).grade) && ((student as any).grade_level || (student as any).grade).toLowerCase().includes(normalizedQuery)) {
      results.push({
        ...searchResult as SearchResult,
        matchType: 'class',
        matchText: (student as any).grade_level || (student as any).grade
      })
      return
    }
  })
    ...
    switch (e.key) {
      ...
      case 'Enter':
        ...
        if (highlightedIndex >= 0 && highlightedIndex < searchResults.length) {
          ...
          const student = selectedResult ? students.find(s => s.id === selectedResult.id) : undefined
          if (student) {
